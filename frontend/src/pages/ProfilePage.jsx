...
import { User, Mail, Lock, Camera, Edit, ArrowLeft, Check, Eye, EyeOff } from "lucide-react";
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    // Обновяваме само ако user съществува и полетата не са празни
  ...
  const handlePasswordChangeSubmit = (e) => {
    ...
    if (formData.newPassword === formData.confirmPassword) {
      // Проверяваме дали name и email не са undefined преди да извикаме updateUser
      if (!formData.name || !formData.email) {
        alert("Името и имейлът не могат да бъдат празни!");
        return;
      }

      // Изпращаме всички необходими полета, за да не се загубят
      updateUser({
        name: formData.name,
        email: formData.email,
  ...
  return (
    <div className='flex flex-col justify-center py-12 sm:px-6 lg:px-8'>
      ...
      <div className='mt-8 sm:mx-auto sm:w-full sm:max-w-4xl'>
        <div className={`flex ${showPasswordChange ? 'justify-between' : 'justify-center'} gap-12`}>
          {/* Форма за профила */}
          <motion.div
              ...
              <form onSubmit={(e) => e.preventDefault()} className='space-y-6'>
                ...
                <div>
                  <button
                    ...
                    className='w-1/2 flex justify-center items-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white
                      bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition duration-150 ease-in-out'
                  >
          ...
          {showPasswordChange && (
            <motion.div
              className='sm:w-full sm:max-w-md mt-12'
